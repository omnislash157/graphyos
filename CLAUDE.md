...
spec_lint.py          the spec linter: a spec is `specs/<n>.md` — headers, `C<n>` contract bullets, scope, hazards, tagged probes, production blocks per host; no prose. Its patterns are the review blockers of #80–#127 condensed: lints the spec must carry with a probe of the right shape, scars it must name. `--patterns` is the build checklist · `--selftest` each rule red on `spec_lint_fixtures/R<n>_bad.md` and silent on `_neg` · the battery lints every tracked spec (`review.py spec-lint`)
specs/                one spec per rung, linted before a byte is built; fired with direct attention against its contract and production blocks, or on the fleet
spec_lint_fixtures/   the linter's proof: `good.md`, and per rule a spec that trips it and a near miss that must not
windows_io_baseline.json  the Windows text seam's ratchet: every engine `write_text` with no `newline=` and every text-mode subprocess with no `encoding=` standing when it was minted; `review.py windows-text-io` refuses a new one
...
review.py             the review battery in front of the reviewer, in the gate: set differences over a parse of the live tree — every advertised `graphy …` command parses against the engine's own argparse, every cited dotted symbol is defined, every argparse dest is read, every path THE FOLDER and THE ENGINE MAP name is on disk or gitignored, no template token is left unfilled, a RECON section's `review round N` rows run in order (a row out of order was written into another rung's section), every sha RECON's newest section names is an ancestor of HEAD, every use of a declared `CACHE_WRITERS` sits in a guard that catches OSError and the driver's `Error` without re-raising, no engine path is built on the literal `substrate` outside a declared `DATA_HOME_DECLARERS` (a data home is the generation the descriptor names), every `repo_cursor`/`cursor_drift` excludes through a declared `CURSOR_EXCLUDERS` call (the build and the check agree on dirt), generation identity is spelled only by `_shared.GENERATION_IDENTITY`, no handler in `cli.py` catches `OSError` in one tuple with the errors a lane names (an OSError nothing named prints `<VERB> FAILED` with its frame, never a refusal), no tracked script runs a non-stdlib `python3 -m` on the host interpreter, no print · logger call · `sys.std*.write` that runs at a module's import carries a non-ASCII character (it runs before any entry point's `utf8_streams` made the streams utf-8 — a cp1252 stderr crashed one hop before `main`), every line of `review_specimens/*.tsv` gives the exit it records, every tracked file reads `text: unset` from `git check-attr` (a core.autocrlf checkout rewrites a shard's digested bytes otherwise), every emitted page holds its contract; `--diff <ref>` adds severance (a deleted symbol a reader outside the diff still names — the reverse-callers class); `--selftest` proves every check red on its own fixture, and the flat run re-proves it as `gate-selftest`; a check that cannot run REFUSES, never a clean zero
...
.gitattributes        `* -text`: git checks out exactly the bytes it holds on every host — a shard is content-addressed and the gate cmp's the svg, the changelog and the arm regions, so a core.autocrlf client (Git for Windows' default) corrupted all of them; `review.py eol-rewritable` reads what git will do, file by file, under every attribute that rewrites a checkout (`text` · `ident` · `working-tree-encoding` · `filter`), from the index's rules and the worktree's
...
smash.py                the minting lane: mint a package into a shard, follow its import ring, parity against a golden; a ring candidate named like a stdlib module that is also a directory under --site-packages is a named `stdlib collision` in the receipt and the RING line, never a silent skip
...
shell/                  what bolts graphy onto a repo: gate.py (walk-before-edit, PreToolUse) · install.py (`--harness claude|codex|cursor`, repeatable) · claude/ (settings.json, GRAPHY.md — the walk's taps and the MEMORY table below, paths filled at install) · codex/hooks.json · cursor/hooks.json (the wiring those harnesses read; the capture reads each transcript by its shape — session_tail.harness_of) · hooks/*.sh and their hooks/*.cmd twins, written LF and CRLF on every host (the .cmd wired for Codex and Cursor on Windows) · README.md
federated_store.py      compile_store → the sqlite store, updated in place when a few shards land (one transaction, equal to a full compile, the generation a sum of row hashes) and untouched when nothing moved; open_for · path_to · spread — the reader; a store one generation format ahead of the engine reading it names the process as the stale side (restart or reconnect), a store behind names the rebuild
...
scene.py                scene · emit_page — the store's hierarchy as a 3D galaxy: arms as glowing clusters at their layer's height, nodes sized by fan-in, crowns labeled, particles across arms; three.js from jsdelivr, the one page that fetches (`draw --emit 3d`, `showcase`'s galaxy.html)
...
| the showcase page: one command, one page of the codebase — the drawing, the pillars, the ring, the MCP block, three questions | `python3 -m graphy showcase <git url \| path> [--out DIR] [--package <name> --site-packages <dir>]` → `SHOWCASE OK` · `index.html` checked + `showcase.txt`; a repo carrying several importable packages refuses with the showcase line to run, one per package |
| eat a repo — the bolt-on in one verb; a git checkout gets `history_graph` minted beside the code (commits · the sessions archive's exchanges welded to the symbols they name), `check` names it stale and names `graphy history --remint` — the history shard alone into a staged generation, every other lane carried byte-for-byte, which `shell install` also runs; never `eat` or `shell install` on a tenant with house lanes | `../.venv/bin/graphy eat --repo <abs> --site-packages <its venv's site-packages \| its node_modules> [--package <name>] [--producer typescript_ast]` → `<repo>/.graphy/` (the served generation is `substrate.gen-…/`, named by `tenant.json`; a re-eat never touches it until the next lands) — a repo with a `package.json` and no importable Python package is eaten as TypeScript |
...
| compile a tenant's store | `python3 -m graphy build --tenant <descriptor> --tenant-id <name> [--full]` — `store fresh, 0 lanes recompiled` when nothing moved, `updated N lane(s) in place` when a few shards landed, the whole store otherwise or under `--full` |
...
| a bare name: the nodes it could mean (exact id · dotted tail · substring), the MCP `hunt` tool's twin | `python3 -m graphy hunt <name> --tenant <descriptor> --tenant-id <name> [--limit N]` — exit 1 when nothing matches |
...
| bolt the hooks and the gate onto an eaten repo; the wiring per harness | `python3 -m graphy shell install --repo <abs> [--harness claude\|codex\|cursor]…` → `.graphy/hooks/*.sh` (+ `*.cmd` on Windows) · `.claude/settings.json` (`.codex/hooks.json` · `.cursor/hooks.json`) · `GRAPHY.md`; the gate: `echo <hook json> \| python3 -m graphy.shell.gate` — exit 2 blocks, the walk to run on stderr |
...
| a rung only the operator can finish: gate it and march on | a closing line `MARCH GATE: <LAW \| MONEY \| ACCOUNT \| PUBLIC \| IRREVERSIBLE> — <the exact step>` — labeled `operator`, the next rung armed; `MARCH GATE: <gate> #N — <step>` gates rung N instead and the armed one stays the lane, a step that only mentions another `#N` is refused; a deferral naming no gate is interrogated |
