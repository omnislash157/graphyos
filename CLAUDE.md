...
federated_store.py      compile_store → the sqlite store, updated in place when a few shards land (one transaction, equal to a full compile, the generation a sum of row hashes) and untouched when nothing moved; open_for · path_to · spread — the reader
