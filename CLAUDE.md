...
review.py             the review battery in front of the reviewer, in the gate: set differences over a parse of the live tree — every advertised `graphy …` command parses against the engine's own argparse, every cited dotted symbol is defined, every argparse dest is read, every path THE FOLDER and THE ENGINE MAP name is on disk or gitignored, no template token is left unfilled, every sha RECON's newest section names is an ancestor of HEAD, a stored answer's key (`RULE_MODULES`) covers the import closure of the code that computed it (`RULE_ROOTS`) and every member pins `SOURCE_SHA` at import, every use of a declared `CACHE_WRITERS` sits in a guard that catches OSError and the driver's `Error` without re-raising, every emitted page holds its contract; `--diff <ref>` adds severance (a deleted symbol a reader outside the diff still names — the reverse-callers class); `--selftest` proves every check red on its own fixture, and the flat run re-proves it as `gate-selftest`; a check that cannot run REFUSES, never a clean zero
...
rebuild.py              the supported multi-lane rebuild: `Lane.mint` (a package this engine's producer walks) · `Lane.placed` (a shard the tenant's own emitter wrote) · `clear_substrate(sub, keep=…)` · `rebuild(…)` — clear → smash → history → init → scheme index → converge --resolve → build → check, a placed lane kept whole across the clear and indexed from the ids it carries; `eat` is the single-ring special case, and the engine never runs a foreign producer
...
cli.py                  graphy eat | init | smash | history | push | pull | index | converge | build | container | estate | walk | bridge | arms | farm | draw | showcase | harness | recon | descend | blast | explain | pillars | refresh | mcp | traversals | shell | check | fanout — exit 0 healthy · 1 audit verdict · 2 never ran
```
