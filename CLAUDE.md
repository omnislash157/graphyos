...
cli.py                  graphy eat | init | smash | history | push | pull | index | converge | build | container | estate | walk | bridge | arms | farm | draw | showcase | descend | blast | explain | pillars | refresh | mcp | traversals | shell | check | fanout — exit 0 healthy · 1 audit verdict · 2 never ran
```
...
| the repo's own record as a shard beside the code: commits · sessions · RECON sections · issues · receipts, a commit's `touches` onto the module ids of the files it changed — walked by the same verbs (`walk` from a session into a module, `explain recon/93`, `estate --sql` over `touches`) | `python3 -m graphy history --repo <abs> --out <data_home>/history_graph [--sessions <abs .claude/recovery/sessions>]` → `HISTORY OK`; the graphy tenant's rebuild mints it as `history_graph` |
