...
| draw it: the pillars, the unit map, one arm, a symbol's neighbourhood — the rebuild lands the atlas at `$T/substrate/atlas/` | `python3 -m graphy draw --tenant $T/tenant.json --tenant-id fastapi --corpus fastapi --pillars --partition $T/partition.json --lr` · `--symbol get_request_handler --radius 2 --emit html --interactive -o page.html` · `--check page.html` |
