...
| how the product changed over time — those sessions oldest first with timestamps, their commits, the RECON sections and issues, the numbers that moved — from the history shard `eat` minted beside the code | `{{python}} -m graphy history "<A>" --with "<B>" --tenant <descriptor> --tenant-id <name> --sessions {{sessions}}` |
| what did we say about this symbol — the exchanges that name it, their sessions oldest first with the same fan-out; no archive read | `{{python}} -m graphy history --symbol <id or dotted tail> --tenant {{desc}} --tenant-id {{tid}}` |
