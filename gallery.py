...
repo's groups and dependencies read from its showcase.txt and a portable MCP example, and `<out>/gallery.json`, the
...
it inside the image so every Railway deploy is a fresh gallery on the current engine. The index explains
graphy in plain language; still static html, nothing fetched.
"""
...
def compose_index(pages: list[dict], *, built_at: str, engine: str, mcp: str = "") -> str:
    """The public index: checked maps, plain-language copy, and setup for the visitor's own repo.

    ``mcp`` remains accepted for callers, but a showcase's build-machine paths do not belong on the homepage.
    """
    green = [p for p in pages if p.get("rc") == 0 and p.get("check") == []]
    rows = []
    for p in green:
        arms = " · ".join(f"<b>{html.escape(a['name'])}</b> <small>({html.escape(a['crown'])})</small>" for a in p["arms"]) or "One module group"
        ring = p.get("ring") or {"count": 0, "names": ""}
        dependencies = (
            f'<div class="ring">Dependencies: {ring["count"]} package(s)'
            f'{(" — " + html.escape(ring["names"])) if ring["names"] else ""}</div>'
            if ring["count"] else "")
        rows.append(
            f'<li><a class="card" href="{html.escape(p["slug"])}/{PAGE}">{html.escape(p["repo"])} <span aria-hidden="true">→</span></a>'
            f'<a class="g3d" href="{html.escape(p["slug"])}/galaxy.html">3D view</a>'
            f'<div class="arms">{arms}</div>'
            f'{dependencies}</li>')
    example = json.dumps({"mcpServers": {"graphy": {
        "command": "graphy", "args": ["mcp", "--repo", "/absolute/path/to/your/repo"]}}}, indent=2)
    mcp_html = f'<pre class="mcp">{html.escape(example)}</pre>'
    n = len(green)
    joined = "\n".join(rows)
    ...
    return (
        ...
        "<meta name=\"description\" content=\"Compile a repo into a graph your agent can query in milliseconds. Explore real codebases in 2D and 3D with graphy.\">\n"
        "<title>graphy — a codebase your agent can query</title>\n"
        "<style>\n"
        ...
        "<span class=\"word\">graphy</span>\n"
        "<span class=\"law\">code, connected</span>\n"
        "</header>\n"
        "<main>\n"
        "<p class=\"kicker\">codebase maps for you and your agent</p>\n"
        "<h1>Compile a repo into a graph your agent can query in milliseconds.</h1>\n"
        "<p class=\"lede\">See how the code connects. Find what a change affects. Explore a 2D map or a 3D view, and let your coding agent query the same structure through MCP.</p>\n"
        "<div class=\"facts\">\n"
        "<span><b>No embeddings</b></span>\n"
        "<span><b>No model picks an edge</b></span>\n"
        "<span><b>Apache 2.0</b></span>\n"
        "</div>\n"
        "<h2>Try a real codebase</h2>\n"
        f"<p class=\"sub\">{n} checked codebase(s). Open a repo for its 2D map, or choose its 3D view. Every connection comes from code structure.</p>\n"
        f"<ul class=\"repos\">\n{joined}\n</ul>\n"
        "<h2>Try it on your repo</h2>\n"
        "<p class=\"sub\">Python, TypeScript, and JavaScript. Install graphy, compile your repo, then open its map.</p>\n"
        "<pre>pip install 'graphyos[typescript]'\n"
        "cd /path/to/your/repo\n"
        "graphy eat . --no-provision\n"
        "graphy showcase . --no-provision</pre>\n"
        "<p class=\"sub\">Open <code>.graphy/showcase/index.html</code>. The <code>--no-provision</code> flag keeps graphy from running the repo's installer.</p>\n"
        "<ol class=\"steps\">\n"
        "<li><div><strong>Compile</strong><small>graphy reads the code's structure and stores its connections under <code>.graphy/</code>.</small></div></li>\n"
        "<li><div><strong>Explore</strong><small>Follow modules and dependencies in the 2D map or the 3D view.</small></div></li>\n"
        "<li><div><strong>Ask your agent</strong><small>Connect through MCP to find symbols, trace dependencies, and see what a change affects.</small></div></li>\n"
        "</ol>\n"
        "<p>For an agent guide with module groups and drawings, run <code>graphy harness --repo .</code> after compiling. It writes <code>GRAPH.md</code> in your repo.</p>\n"
        "<p>Want a map of a public repo? <a href=\"https://github.com/omnislash157/graphyos/issues/new\">Open an issue with its GitHub URL</a> and the automation replies with its structure and the commands to generate a map.</p>\n"
        "<h2>Connect your coding agent</h2>\n"
        "<p class=\"sub\">After installing graphy and compiling your repo, paste this into <code>.mcp.json</code> (Claude Code) or your client's MCP settings. Replace <code>/absolute/path/to/your/repo</code> with your repo's full path, and make sure the client can find the installed <code>graphy</code> command.</p>\n"
        f"{mcp_html}\n"
