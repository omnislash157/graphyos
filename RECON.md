...
## 128 · A REBUILD NO LONGER TAKES THE DOOR DOWN — the clear removed the served store, the descriptor and the ring, so `graphy mcp` refused `no compiled store` from the clear until build landed; the clear now keeps the store and the ring, the next descriptor is staged and lands in one rename after build, and a door opened mid-rebuild finds the last good store named STALE (2026-09-14 · graphyos issue 98)

- `cli._clear_substrate` keeps `.mesh_store_*` and `ring.json`, and writes `federated_store.REBUILD_MARKER` when a store is served. `cli.staged_descriptor` · `cli.land_descriptor`: `eat` and `rebuild.rebuild` run init · converge · build against `.tenant.json.next`, rename it over the served descriptor, remove the marker and prune every store no roster names, then `check`.
- `federated_store.open_for`: inputs that cannot be read whole while the marker stands are a rebuild in flight — the last good store is served under `--on-stale warn` with the marker's timestamp, refused as STALE under `refuse`. With no marker, an unreadable input stays a torn substrate and refuses even under warn (`test_RED_unmeasurable_input_raises_never_serves`, unchanged).
- Not covered on this box: build's final `os.replace` over a store an MCP client holds open on Windows.

| check | result |
|---|---|
| the done test | `cd engine && ../.venv/bin/python -m pytest -q -k store_stays_servable_through_a_rebuild` → 1 passed: the MCP tools open at the clear and before smash · init · converge · build · check (served under warn, STALE under refuse), fresh after, the orphan store pruned, no staged descriptor or marker left, a torn registry refused under warn. With the three engine files stashed it fails `no ring receipt` at the clear |
| the floor · the gate | `python3 -m pytest -q` green · `standalone_check.sh` → `GRAPHY_STANDALONE_OK` |
| review | NOT YET REVIEWED — committed on the operator's ruling so the tree is clean for the fleet's baton; #98 stays open until a review round reads SHIP |
